  ...
  const handleLoginSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.user) {
        ...
        router.replace(response.data.user.isProfileComplete ? "/Dashboard" : "/Generate");
      } else {
            ...
            {isForgotPassword ? (
                <>
                    <div className="text-center mb-8">
                        <h1 className={`text-2xl font-bold ${styles.textColor}`}>Reset Password</h1>
                        <p className={`text-sm mt-2 ${styles.subTextColor}`}>We&apos;ll send you a link to reset your password.</p>
                    </div>
                    <form className="space-y-4" onSubmit={handleForgotPasswordSubmit}>
                        <div className="space-y-1.5">
                            <label className={`text-sm font-medium ${theme === 'dark' ? 'text-gray-300' : 'text-slate-700'}`}>Email Address</label>
                            <input
                                type="email"
                        ...
                        <button
                            type="submit"
                            disabled={loading}
                            className={`${styles.btnBase} ${styles.btnTheme} flex items-center justify-center gap-2 mt-4`}
                        >
                        ...
                        <button
                            ...
                            className={`w-full text-sm mt-4 hover:text-teal-500 transition-colors ${styles.subTextColor}`}
                        >
                            Back to Login
                        </button>
                    </form>

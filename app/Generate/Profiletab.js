...
export default function Profiletab({ handle, setHandle, profile, setprofile, script, setScript, location, setLocation, profession, setProfession, theme, handleAvailable, setHandleAvailable, handleChecking, setHandleChecking }) {
  ...
  return (
    <div className="space-y-8 animate-enter">
      ...
      <div>
        ...
        {/* Drag & Drop Area */}
        <div
          className={`relative group flex flex-col items-center justify-center w-full h-48 border-2 border-dashed rounded-2xl transition-all cursor-pointer overflow-hidden ${dragActive
      ...
      <div>
        ...
        <div className="flex shadow-sm rounded-xl overflow-hidden">
          ...
          <input
            ...
            className={`flex-1 px-4 py-3 text-sm outline-none border border-l-0 rounded-none rounded-r-xl transition-all
            ${handleAvailable === false ? "border-red-500" : ""}
            ${handleAvailable === true ? "border-green-500" : ""}
            ${handleAvailable === null ? colors.input : ""}
            `}
        ...
        {/* Status messages */}
        {handleChecking && (
          <p className="text-yellow-500 text-xs mt-1">Checking…</p>
        )}

        {handleAvailable === true && (
          <p className="text-green-600 text-xs mt-1"> Handle is available</p>
        )}

        {handleAvailable === false && (
          <p className="text-red-600 text-xs mt-1"> Handle already taken</p>
        )}
      </div>

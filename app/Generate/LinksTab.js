...
export default function LinksTab({ links = [], setlinks, addlink, removeLink, theme }) {
    ...
    // --- Styles ---
    const inputClasses = `block w-full rounded-lg text-sm p-2.5 outline-none border transition-all ${theme === 'dark'
    ...
    const labelClasses = `absolute -top-2 left-2 px-1 text-[10px] font-bold uppercase tracking-wider z-10 ${theme === 'dark' ? 'bg-[#111] text-gray-500' : 'bg-white text-gray-400'
        }`;
    ...
    return (
        <div className="space-y-6 animate-enter">
            ...
            <div className="flex justify-between items-center mb-2">
                ...
                <button
                    onClick={addlink}
                    className={`inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-lg text-white transition-colors shadow-lg shadow-teal-500/20 ${theme === 'dark' ? 'bg-teal-600 hover:bg-teal-500' : 'bg-black hover:bg-gray-800'
                        }`}
                >
            ...
            <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                {Array.isArray(links) && links.map((link, index) => (
                    <div
                        ...
                        className={`flex gap-3 items-start p-4 rounded-xl border group transition-all duration-200 relative cursor-move ${theme === 'dark'
                    ...
                    >
                        {/* Drag Handle */}
                        <div
                            className={`mt-3 cursor-grab active:cursor-grabbing ${theme === 'dark' ? 'text-gray-600 hover:text-gray-300' : 'text-gray-400 hover:text-gray-600'
                                }`}
                        ...
                        <button
                            ...
                            onMouseDown={(e) => e.stopPropagation()}
                            className={`p-2 mt-1 rounded-lg transition-colors ${theme === 'dark'
            ...
            {(!links || links.length === 0) && (
                <div className={`flex flex-col items-center justify-center py-12 border-2 border-dashed rounded-2xl ${theme === 'dark' ? 'border-white/10 bg-white/5' : 'border-gray-200 bg-gray-50'
                    }`}>

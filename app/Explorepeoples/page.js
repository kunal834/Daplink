...
const UserProfile = ({ params }) => {
  ...
  const handleOpenChat = (user) => {
    ...
    setActiveChat({
      _id: user._id,
      handle: user?.daplinkID?.handle || user?.handle || "Incognito",
      avatar: user?.daplinkID?.profile || user?.avatar || user?.profile,
    });
  };

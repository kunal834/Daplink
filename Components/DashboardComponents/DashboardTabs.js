...
export const LinksTab = ({ isDarkMode, links, setLinks }) => {
  ...
  const addLink = (e) => {
    ...
    const newLink = { id: Date.now(), title: newLinkTitle, url: newLinkUrl.startsWith('http') ? newLinkUrl : `https://${newLinkUrl}`, active: true, clicks: 0, createdAt: new Date() };
    setLinks([newLink, ...links]);
    setNewLinkTitle(''); setNewLinkUrl(''); setIsAdding(false);
  };

  const deleteLink = (id) => setLinks(links.filter(l => l.id !== id));
  const toggleLinkActive = (id) => setLinks(links.map(l => l.id === id ? { ...l, active: !l.active } : l));

  return (
    <div className="space-y-6 animate-in fade-in duration-300">
       <div className="flex items-center justify-between mb-4">
          <div><h2 className="text-2xl font-bold tracking-tight">Your Links</h2><p className={`text-sm ${isDarkMode ? 'text-zinc-500' : 'text-zinc-500'}`}>Manage the links displayed on your page.</p></div>
          {!isAdding && <button onClick={() => setIsAdding(true)} className={`px-5 py-2.5 rounded-xl text-sm font-semibold flex items-center gap-2 shadow-lg transition-all active:scale-95 group ${isDarkMode ? 'bg-indigo-600 hover:bg-indigo-500 text-white shadow-indigo-500/20' : 'bg-black hover:bg-zinc-800 text-white shadow-zinc-500/20'}`}><Plus className="w-4 h-4 group-hover:rotate-90 transition-transform" /> Add Link</button>}
       </div>
      ...
      {isAdding && (
        <div className={`rounded-3xl p-6 shadow-xl border animate-in slide-in-from-top-4 duration-300 ${isDarkMode ? 'bg-zinc-900 border-zinc-800 shadow-none' : 'bg-white shadow-zinc-200/50 border-zinc-100 ring-1 ring-zinc-50'}`}>
           <div className="flex justify-between items-center mb-6"><h3 className="font-bold text-lg">Create New Link</h3><button onClick={() => setIsAdding(false)} className={`w-8 h-8 rounded-full flex items-center justify-center transition-colors ${isDarkMode ? 'bg-zinc-800 hover:bg-zinc-700 text-zinc-400' : 'bg-zinc-50 hover:bg-zinc-100 text-zinc-400 hover:text-zinc-600'}`}><X className="w-5 h-5" /></button></div>
           <form onSubmit={addLink} className="space-y-5">
              <div className="space-y-2"><label className={`text-xs font-bold ml-1 ${isDarkMode ? 'text-zinc-500' : 'text-zinc-500'}`}>TITLE</label><input autoFocus type="text" placeholder="e.g., My Portfolio" className={`w-full px-4 py-3.5 rounded-xl border-2 transition-all text-sm font-medium outline-none ${isDarkMode ? 'bg-zinc-950 border-zinc-800 focus:border-indigo-500 text-white placeholder:text-zinc-600' : 'bg-zinc-50 border-transparent focus:bg-white focus:border-zinc-900 placeholder:text-zinc-400'}`} value={newLinkTitle} onChange={(e) => setNewLinkTitle(e.target.value)}/></div>
              <div className="space-y-2"><label className={`text-xs font-bold ml-1 ${isDarkMode ? 'text-zinc-500' : 'text-zinc-500'}`}>URL</label><input type="text" placeholder="e.g., https://dribbble.com/me" className={`w-full px-4 py-3.5 rounded-xl border-2 transition-all text-sm font-medium outline-none ${isDarkMode ? 'bg-zinc-950 border-zinc-800 focus:border-indigo-500 text-white placeholder:text-zinc-600' : 'bg-zinc-50 border-transparent focus:bg-white focus:border-zinc-900 placeholder:text-zinc-400'}`} value={newLinkUrl} onChange={(e) => setNewLinkUrl(e.target.value)}/></div>
              <div className="flex items-center gap-3 pt-2"><button type="submit" className={`flex-1 py-3.5 rounded-xl font-bold text-sm shadow-lg transition-all active:scale-[0.98] ${isDarkMode ? 'bg-indigo-600 hover:bg-indigo-500 text-white shadow-indigo-900/20' : 'bg-zinc-900 hover:bg-black text-white shadow-zinc-900/10'}`}>Create Link</button><button type="button" onClick={() => setIsAdding(false)} className={`px-6 py-3.5 border rounded-xl font-bold text-sm transition-colors ${isDarkMode ? 'bg-zinc-950 border-zinc-800 hover:bg-zinc-900 text-zinc-400' : 'bg-white border-zinc-200 hover:bg-zinc-50 text-zinc-600'}`}>Cancel</button></div>
           </form>
        </div>
      )}
      ...
      <div className="space-y-3">
         {links.length === 0 && !isAdding && (
           <div className={`text-center py-20 rounded-3xl border border-dashed transition-colors group cursor-pointer ${isDarkMode ? 'bg-zinc-900/50 border-zinc-800 hover:border-zinc-700' : 'bg-white border-zinc-200 hover:border-zinc-300'}`} onClick={() => setIsAdding(true)}>
             <div className={`w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-5 transition-all duration-300 group-hover:scale-110 ${isDarkMode ? 'bg-zinc-800 text-zinc-600 group-hover:text-indigo-400' : 'bg-zinc-50 text-zinc-300 group-hover:text-indigo-400'}`}><LinkIcon className="w-10 h-10" /></div>
             <h3 className="font-bold text-lg mb-1">Start building your page</h3>
             <p className={`text-sm max-w-xs mx-auto ${isDarkMode ? 'text-zinc-500' : 'text-zinc-500'}`}>Add your first link to show the world what you're made of.</p>
           </div>
         )}
         {links.map((link) => (
           <div key={link.id} className={`rounded-2xl p-4 pl-3 shadow-sm border transition-all flex items-center gap-4 group ${isDarkMode ? 'bg-zinc-900 border-zinc-800 hover:border-zinc-700' : 'bg-white border-zinc-200/60 hover:border-zinc-300 hover:shadow-md'}`}>
              <div className={`cursor-grab p-2 rounded-lg transition-colors ${isDarkMode ? 'text-zinc-600 hover:bg-zinc-800 hover:text-zinc-300' : 'text-zinc-300 hover:text-zinc-600 hover:bg-zinc-50'}`}><GripVertical className="w-5 h-5" /></div>
              <div className="flex-1 min-w-0"><h4 className="font-bold truncate text-sm">{link.title}</h4><div className={`flex items-center gap-1.5 text-xs ${isDarkMode ? 'text-zinc-500' : 'text-zinc-400'}`}><Globe className="w-3 h-3" /><span className="truncate">{link.url}</span></div></div>
              <div className="flex items-center gap-3 pr-2">
                 <div className={`text-xs font-bold px-2 py-1 rounded-md hidden sm:block ${isDarkMode ? 'bg-zinc-800 text-zinc-500' : 'bg-zinc-50 text-zinc-400'}`}>{link.clicks || 0} clicks</div>
                 <button onClick={() => toggleLinkActive(link.id)} className={`w-11 h-6 rounded-full relative transition-colors duration-300 ${link.active ? (isDarkMode ? 'bg-indigo-600' : 'bg-zinc-900') : (isDarkMode ? 'bg-zinc-800' : 'bg-zinc-200')}`}><div className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all duration-300 shadow-sm ${link.active ? 'left-6' : 'left-1'}`} /></button>
                 <button onClick={() => deleteLink(link.id)} className={`p-2 rounded-lg transition-colors ${isDarkMode ? 'text-zinc-600 hover:text-red-400 hover:bg-red-900/20' : 'text-zinc-300 hover:text-red-500 hover:bg-red-50'}`}><Trash2 className="w-4 h-4" /></button>
              </div>
           </div>
         ))}
      </div>
...
export const SkillSwapTab = ({ isDarkMode }) => {
  const skillSwaps = [
...
export const JobFinderTab = ({ isDarkMode }) => {
  const jobs = [
    { id: 1, role: "UX Designer", company: "FinTech AI", rate: "$5k/project", type: "Contract" },
    { id: 2, role: "Video Editor", company: "Creator Studio", rate: "$2k/month", type: "Part-time" },
    { id: 3, role: "Frontend Dev", company: "Web3 Startup", rate: "$80/hr", type: "Freelance" },
  ];
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
        <div className="flex justify-between items-end">
          <div><h2 className="text-2xl font-bold tracking-tight">Job Finder</h2><p className={`${isDarkMode ? 'text-zinc-500' : 'text-zinc-500'} text-sm`}>AI-matched freelance gigs based on your bio.</p></div>
          <span className="text-xs font-bold bg-indigo-500 text-white px-2 py-1 rounded-md animate-pulse">3 New Matches</span>
        </div>
        <div className="space-y-4">
            {jobs.map((job) => (
              <div key={job.id} className={`p-6 rounded-3xl border flex items-center gap-6 transition-all group cursor-pointer ${isDarkMode ? 'bg-zinc-900 border-zinc-800 hover:border-indigo-500/50' : 'bg-white border-zinc-200 hover:border-indigo-400 hover:shadow-lg'}`}>
                  <div className={`w-14 h-14 rounded-2xl flex items-center justify-center ${isDarkMode ? 'bg-zinc-800 text-zinc-400' : 'bg-zinc-50 text-zinc-600'}`}><Briefcase className="w-6 h-6" /></div>
                  <div className="flex-1"><h4 className="font-bold text-lg group-hover:text-indigo-500 transition-colors">{job.role}</h4><p className={`text-sm ${isDarkMode ? 'text-zinc-500' : 'text-zinc-500'}`}>{job.company} • {job.type}</p></div>
                  <div className="text-right"><p className={`font-bold text-lg ${isDarkMode ? 'text-white' : 'text-zinc-900'}`}>{job.rate}</p><button className="text-xs font-bold text-indigo-500 hover:underline">Apply Now</button></div>
              </div>
            ))}
        </div>
    </div>
